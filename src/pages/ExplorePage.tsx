...
export const ExplorePage: React.FC<ExplorePageProps> = ({ onNavigate, onSelectExperience }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8 text-left">
      ...
      {/* Header */}
      <div className="space-y-2 border-b border-[#E7DBB8] pb-6">
        <div className="flex items-center gap-2 text-xs font-bold text-[#C25E2E] uppercase tracking-wider">
          <Compass className="w-4 h-4" />
          <span>ค้นหาทริปและกิจกรรมวิถีไทย</span>
        </div>
        <h1 className="text-3xl sm:text-4xl font-black text-[#5A3A28]">
          ประสบการณ์ท่องเที่ยวโดยชุมชน (CBT Directory)
        </h1>
        <p className="text-sm text-[#5A3A28]/80 max-w-2xl">
          เลือกทริปที่ใช่ สัมผัสวัฒนธรรมแท้ และมั่นใจว่าค่าทริปของคุณถูกกระจายสู่ชุมชนอย่างโปร่งใส 100%
        </p>
      </div>

      {/* SEARCH AND FILTER BAR */}
      <div className="bg-[#FFFDF8] p-5 rounded-3xl border-2 border-[#E7DBB8] shadow-md space-y-4">
        
        {/* Top Controls */}
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
          
          {/* Search Input */}
          <div className="md:col-span-5 bg-[#FAF4E9] p-3 rounded-2xl border border-[#E7DBB8] flex items-center gap-2">
            ...
            <input 
              ...
              className="bg-transparent text-xs sm:text-sm font-semibold text-[#5A3A28] focus:outline-hidden w-full"
            />
          </div>

          {/* Region Select */}
          <div className="md:col-span-3 bg-[#FAF4E9] p-3 rounded-2xl border border-[#E7DBB8] flex items-center gap-2">
          ...
          {/* Sort Selector */}
          <div className="md:col-span-4 bg-[#FAF4E9] p-3 rounded-2xl border border-[#E7DBB8] flex items-center gap-2">
            <ArrowUpDown className="w-4 h-4 text-[#C25E2E] shrink-0" />
            <span className="text-xs font-bold text-[#5A3A28] shrink-0">เรียงตาม:</span>
            <select
              value={sortBy}
        ...
        <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-[#E7DBB8]/60">
          <span className="text-xs font-bold text-[#5A3A28] mr-2">หมวดหมู่:</span>
          {[
              ...
              <button
                ...
                className={`px-3.5 py-1.5 rounded-full text-xs font-bold transition-all cursor-pointer ${
                  isActive
                    ...
                    : 'bg-[#F2E6C9]/60 hover:bg-[#E7DBB8] text-[#5A3A28]'
                }`}
              >
                {cat.label}
              </button>
      ...
      {/* Results Counter Bar */}
      <div className="flex items-center justify-between text-xs font-bold text-[#5A3A28]/80">
        <span>พบ {filteredExperiences.length} กิจกรรมที่ตรงกับการค้นหา</span>
        <button 
          onClick={() => onNavigate('transparency')}
          className="text-[#C25E2E] hover:underline flex items-center gap-1 cursor-pointer"
        >
          ...
          >
            <div>
              ...
              <div className="p-6 space-y-3">
                <p className="text-xs font-bold text-[#5E7045] flex items-center gap-1">
                ...
                <p className="text-xs text-[#5A3A28]/75 line-clamp-2">
                  {exp.subtitleTh}
                </p>

                {/* Transparency Summary Module */}
                <div className="bg-[#FAF4E9] p-3 rounded-2xl border border-[#E7DBB8] space-y-1.5">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-semibold text-[#5A3A28]">สัดส่วนเงินส่งตรงชุมชน:</span>
                    <span className="font-black text-[#5E7045]">
            ...
            <div className="p-6 pt-0 border-t border-[#E7DBB8]/40 mt-4 flex items-center justify-between">
              <div>
                <span className="text-[10px] font-semibold text-[#5A3A28]/60 block">ราคา/ท่าน</span>
                <span className="text-xl font-black text-[#C25E2E]">
                  {exp.priceThbPerPerson.toLocaleString()} <span className="text-xs font-normal text-[#5A3A28]">THB</span>
                </span>
              </div>
